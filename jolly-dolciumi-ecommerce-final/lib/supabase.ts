import type { Address, CartEntry, Coupon, Order, Product, Profile, Quote, SavedAddress, Settings } from "./shop";
...
declare global { interface Window { __JOLLY_CONFIG__?: { supabaseUrl?: string; supabaseAnonKey?: string } } }
...
export class ShopApi {
  ...
  async signUp(email: string, password: string, fullName: string) {
    const s = await this.raw<Session>(`/auth/v1/signup?redirect_to=${encodeURIComponent(location.origin + "/")}`, { method: "POST", body: JSON.stringify({ email: email.trim(), password, data: { full_name: fullName.trim() } }) });
  ...
  reset(email: string) { return this.raw(`/auth/v1/recover?redirect_to=${encodeURIComponent(location.origin + "/")}`, { method: "POST", body: JSON.stringify({ email: email.trim() }) }); }
  ...
  products() { return this.all<Product>("products", "select=*&order=featured.desc,created_at.desc,id.asc"); }
  ...
  staffUser(p: Profile) { return this.rpc("staff_user_update", { p_user_id: p.id, p_name: p.full_name, p_phone: p.phone || "", p_role: p.role, p_blocked: !!p.is_blocked }); }
