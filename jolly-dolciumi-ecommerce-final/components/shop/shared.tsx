...
import { Candy, Cookie, GlassWater, IceCreamBowl, Package, Gift, Minus, Plus, Heart, ShoppingBag, ArrowUpRight } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Dialog, DialogContent, DialogTitle, DialogDescription } from "@/components/ui/dialog";
import { Input } from "@/components/ui/input";
import { type Product, money, productImage } from "@/lib/shop";

export function Brand({ small = false }: { small?: boolean }) { return <span className={`brand ${small ? "brand-small" : ""}`}><strong>Jolly<span>®</span></strong><small>DOLCIUMI & MOLTO ALTRO</small></span>; }
...
export function ProductCard({ product: p, inCart, favorite, busy, onOpen, onAdd, onFavorite }: { product: Product; inCart: number; favorite: boolean; busy: boolean; onOpen: () => void; onAdd: (quantity: number) => void; onFavorite: () => void }) {
  const [q, setQ] = useState(1);
  const available = Math.max(0, Math.min(p.stock - inCart, 99 - inCart));
  ...
  return <article className="product-card">
    ...
    <div className="card-copy"><p className="product-category">{p.category}{p.weight_label ? ` · ${p.weight_label}` : ""}</p><button className="product-title" onClick={onOpen}><h3>{p.name}</h3></button><p className="product-excerpt">{p.description}</p>
      <div className="price-line">{quote ? <strong className="quote-price">Su preventivo</strong> : <><strong>{money(Number(p.price))}</strong>{saving > 0 && <del>{money(Number(p.compare_at_price))}</del>}</>}</div>
      <p className={`stock ${!quote && available === 0 ? "stock-empty" : ""}`}>{quote ? "Soluzioni per la tua attività" : p.stock === 0 ? "Momentaneamente esaurito" : inCart > 0 ? `${inCart} nel carrello · ${available} ancora disponibili` : p.stock < 6 ? `Ultimi ${p.stock} pezzi disponibili` : `${p.stock} pezzi disponibili`}</p>
      <div className="card-actions">{quote ? <Button className="btn btn-outline full" onClick={onOpen}>Richiedi informazioni <ArrowUpRight size={17} /></Button> : <><Quantity value={quantity} max={available} disabled={busy || available === 0} onChange={setQ} label={`Quantità ${p.name}`} /><Button className="btn card-add" disabled={busy || available === 0} onClick={() => onAdd(quantity)} aria-label={`Aggiungi ${quantity} ${p.name} al carrello`}><Plus size={18} /><span>Aggiungi</span></Button></>}</div>
    </div>
  </article>;
}
export function ProductModal({ product: p, inCart, busy, onClose, onAdd, onQuote }: { product: Product; inCart: number; busy: boolean; onClose: () => void; onAdd: (q: number) => void; onQuote: (message: string, phone: string) => Promise<void> }) {
  ...
  const available = Math.max(0, Math.min(p.stock - inCart, 99 - inCart)); const quote = p.purchase_mode === "quote";
