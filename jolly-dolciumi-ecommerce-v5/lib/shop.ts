...
export type BundleItem = { label: string; quantity: number };
...
export function quantityDiscount(product: Product, quantity: number) {
  return (product.quantity_discounts || []).filter(d => Number(d.min_quantity) <= quantity).sort((a, b) => Number(b.min_quantity) - Number(a.min_quantity))[0]?.percent_off || 0;
}
...
export function parseBundleItems(value: string): BundleItem[] {
  return value.split(",").map(part => part.trim()).filter(Boolean).map(part => { const cut = part.lastIndexOf(":"); const label = (cut > 0 ? part.slice(0, cut) : part).trim(); const quantity = Number(cut > 0 ? part.slice(cut + 1).trim() : "1"); return { label, quantity: Math.floor(quantity) }; }).filter(x => x.label.length >= 2 && Number.isInteger(x.quantity) && x.quantity > 0 && x.quantity < 100);
}
