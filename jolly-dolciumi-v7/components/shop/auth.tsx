...
export function Auth({ api, onUser, initialError = "" }: { api: ShopApi | null; onUser: (p: Profile) => void; initialError?: string }) {
  const [code,setCode] = useState(""); const [resetStage,setResetStage] = useState<"request"|"verify"|"password">("request");
  ...
  async function submit(e: FormEvent) {
    ...
    try {
      if (mode === "reset") {
        if (resetStage === "request") { await api.reset(email); setResetStage("verify"); setSuccess(true); setMessage("Se l’indirizzo è registrato, riceverai un codice via email. Inseriscilo qui."); }
        else { await api.verifyRecovery(email,code); setResetStage("password"); }
      }
  ...
  if(mode === "reset" && resetStage === "password" && api) return <PasswordSetup api={api} onDone={()=>{api.profile().then(onUser).catch(e=>{setMessage(errorMessage(e));setResetStage("request");setMode("login");});}}/>;
  return <main className="auth-page"><section className="auth-story"><Brand /><div className="auth-story-copy"><span className="eyebrow"><Sparkles size={16} /> BENVENUTO NEL MONDO JOLLY</span><h1>Tutto il buono.<br /><em>In un solo posto.</em></h1><p>Dalle caramelle alle patatine, dalle bibite alle attrezzature per gelati e granite. Il tuo prossimo assortimento comincia qui.</p><div className="auth-category-list"><span>Dolci & cioccolato</span><span>Snack & bibite</span><span>Gelati & attrezzature</span></div></div><div className="auth-photo"><img src="/assortment.png" alt="Assortimento illustrativo di snack, dolci e bibite" /><span className="image-caption">Una pausa, mille possibilità.</span></div><div className="auth-footnote">IL LATO BUONO DELLA TUA GIORNATA.</div></section>
    <section className="auth-panel"><div className="auth-mobile-brand"><Brand /></div><div className="auth-form-wrap"><span className="pill"><ShieldCheck size={16} /> Catalogo riservato</span><h2>{mode === "login" ? "Ciao, bentornato." : mode === "signup" ? "Piacere, Jolly." : "Ripartiamo da qui."}</h2><p className="muted">{mode === "login" ? "Accedi al tuo account per scoprire il catalogo e fare il tuo ordine." : mode === "signup" ? "Crea il tuo account cliente e trova il tuo prossimo assortimento." : "Ricevi un codice via email e scegli la nuova password qui, senza aprire link."}</p>
      {!api ? <div className="notice error" role="alert">Configurazione Supabase mancante. Verifica il file public/config.js.</div> : <form onSubmit={submit} className="form-stack">
        ...
        <Field label="Indirizzo email"><Input type="email" autoComplete="email" value={email} onChange={e => setEmail(e.target.value)} readOnly={mode === "reset" && resetStage === "verify"} required maxLength={254} placeholder="nome@esempio.it" /></Field>
        {mode === "reset" && resetStage === "verify" && <><Field label="Codice ricevuto via email"><Input autoComplete="one-time-code" inputMode="numeric" pattern="[0-9]{6,10}" minLength={6} maxLength={10} value={code} onChange={e=>setCode(e.target.value.replace(/\D/g,""))} required /></Field><button type="button" className="text-button" disabled={busy} onClick={()=>{setResetStage("request");setMessage("");setCode("");}}>Cambia email o richiedi un altro codice</button></>}
        ...
        {mode === "login" && <button className="text-button auth-forgot" type="button" onClick={() => {setMode("reset");setResetStage("request");setMessage("");}}>Password dimenticata?</button>}
        {message && <p className={`notice ${success ? "success" : "error"}`} role={success ? "status" : "alert"}>{message}</p>}
        <Button type="submit" className="btn full btn-lg" disabled={busy}>{busy ? "Un momento…" : mode === "login" ? "Accedi al catalogo" : mode === "signup" ? "Crea il tuo account" : resetStage === "verify" ? "Verifica codice" : "Invia codice"}<ArrowRight size={18} /></Button>
      </form>}
