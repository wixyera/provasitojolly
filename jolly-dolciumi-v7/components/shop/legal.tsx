...
export function LegalPage({kind}:{kind:"privacy"|"terms"}){
 ...
 return <main className="policy-page page-wrap"><Link href="/" className="policy-back">← Torna a Jolly</Link><h1>{kind==="privacy"?"Informativa privacy":"Condizioni di vendita"}</h1><p>{company}</p>{error?<p role="alert">{error}</p>:<div style={{whiteSpace:"pre-wrap",lineHeight:1.8}}>{text||"Caricamento…"}</div>}</main>;
}
