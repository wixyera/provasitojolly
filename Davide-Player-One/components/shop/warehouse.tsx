...
export function Warehouse({ api, products, onRefresh }: { api: ShopApi; products: Product[]; onRefresh: () => Promise<void> }) {
  ...
  useEffect(() => {
    ...
    const start = async () => {
      ...
      try { if(DetectorClass) detector = new DetectorClass({ formats: ["qr_code"] }); } catch { /* Software decoder below. */ }
      ...
      const scan = async () => {
        if (cancelled || !videoRef.current) { stream?.getTracks().forEach(track=>track.stop()); return; }
        try {
          if (performance.now()-last < 150) { frame=requestAnimationFrame(scan); return; }
          last=performance.now();
          let raw: string | undefined;
          if(detector) { try { raw=(await detector.detect(videoRef.current)).find(x=>x.rawValue)?.rawValue; } catch { detector=null; } }
          if(!raw && context && videoRef.current.videoWidth) {
            canvas.width=Math.min(640,videoRef.current.videoWidth); canvas.height=Math.round(canvas.width*videoRef.current.videoHeight/videoRef.current.videoWidth);
            context.drawImage(videoRef.current,0,0,canvas.width,canvas.height);
            const pixels=context.getImageData(0,0,canvas.width,canvas.height);
            raw=jsQR(pixels.data,pixels.width,pixels.height)?.data;
          }
          if (raw) {
